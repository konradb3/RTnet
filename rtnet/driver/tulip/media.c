...
void tulip_select_media(struct rtnet_device *rtdev, int startup)
{
	...
	if (mtable) {
		...
		case 0:					/* 21140 non-MII xcvr. */
			if (tulip_debug > 1)
				/*RTnet*/rtos_print(KERN_DEBUG "%s: Using a 21140 non-MII transceiver"
		...
		case 2: case 4: {
			...
			if (startup && mtable->has_reset) {
				...
				if (tulip_debug > 1)
					/*RTnet*/rtos_print(KERN_DEBUG "%s: Resetting the transceiver.\n",
						   rtdev->name);
				for (i = 0; i < rst[0]; i++)
					outl(get_u16(rst + 1 + (i<<1)) << 16, ioaddr + CSR15);
			}
			if (tulip_debug > 1)
				/*RTnet*/rtos_print(KERN_DEBUG "%s: 21143 non-MII %s transceiver control "
					   "%4.4x/%4.4x.\n",
			...
			if (tulip_debug > 1)
				/*RTnet*/rtos_print(KERN_DEBUG "%s:  Setting CSR15 to %8.8x/%8.8x.\n",
		...
		case 1: case 3: {
			...
			if (tmp_info && startup < 2) {
				...
				if (tulip_debug > 1)
					/*RTnet*/rtos_print(KERN_DEBUG "%s:  Advertising %4.4x on MII %d.\n",
		...
		default:
			/*RTnet*/rtos_print(KERN_DEBUG "%s:  Invalid media table selection %d.\n",
					   rtdev->name, mleaf->type);
			new_csr6 = 0x020E0000;
		}
		if (tulip_debug > 1)
			/*RTnet*/rtos_print(KERN_DEBUG "%s: Using media type %s, CSR12 is %2.2x.\n",
				   rtdev->name, medianame[rtdev->if_port],
				   inl(ioaddr + CSR12) & 0xff);
	} else if (tp->chip_id == DC21041) {
		int port = rtdev->if_port <= 4 ? rtdev->if_port : 0;
		if (tulip_debug > 1)
			/*RTnet*/rtos_print(KERN_DEBUG "%s: 21041 using media %s, CSR12 is %4.4x.\n",
		...
		if (tulip_debug > 1)
			/*RTnet*/rtos_print(KERN_DEBUG "%s: PNIC PHY status is %3.3x, media %s.\n",
		...
		if (tulip_debug > 1)
			/*RTnet*/rtos_print(KERN_DEBUG "%s: 21040 media type is %s, CSR12 is %2.2x.\n",
		...
		if (tulip_media_cap[rtdev->if_port] & MediaIsMII) {
			...
			new_csr6 = 0x03860000;
		if (tulip_debug > 1)
			/*RTnet*/rtos_print(KERN_DEBUG "%s: No media description table, assuming "
...
int tulip_check_duplex(struct rtnet_device *rtdev)
{
	...
	if (tulip_debug > 1)
		/*RTnet*/rtos_print(KERN_INFO "%s: MII status %4.4x, Link partner report "
			   "%4.4x.\n", rtdev->name, bmsr, lpa);
	if (bmsr == 0xffff)
		return -2;
	if ((bmsr & BMSR_LSTATUS) == 0) {
		...
		if ((new_bmsr & BMSR_LSTATUS) == 0) {
			if (tulip_debug  > 1)
				/*RTnet*/rtos_print(KERN_INFO "%s: No link beat on the MII interface,"
					   " status %4.4x.\n", rtdev->name, new_bmsr);
			return -1;
		}
	}
	...
	if (new_csr6 != tp->csr6) {
		...
		if (tulip_debug > 0)
			/*RTnet*/rtos_print(KERN_INFO "%s: Setting %s-duplex based on MII"
...
void __devinit tulip_find_mii (struct rtnet_device *rtdev, int board_idx)
{
	...
	for (phyn = 1; phyn <= 32 && phy_idx < sizeof (tp->phys); phyn++) {
		...
		/*RTnet*/rtos_print(KERN_INFO "tulip%d:  MII transceiver #%d "
			"config %4.4x status %4.4x advertising %4.4x.\n",
			board_idx, phy, mii_reg0, mii_status, mii_advert);

		/* Fixup for DLink with miswired PHY. */
		if (mii_advert != to_advert) {
			/*RTnet*/rtos_print(KERN_DEBUG "tulip%d:  Advertising %4.4x on PHY %d,"
	...
	if (tp->mtable && tp->mtable->has_mii && phy_idx == 0) {
		/*RTnet*/rtos_print(KERN_INFO "tulip%d: ***WARNING***: No MII transceiver found!\n",
			board_idx);
		tp->phys[0] = 1;
	}
}
