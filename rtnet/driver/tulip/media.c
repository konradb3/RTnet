...
int tulip_mdio_read(struct rtnet_device *rtdev, int phy_id, int location)
{
	...
	flags = rt_spin_lock_irqsave(&tp->mii_lock);
	if (tp->chip_id == LC82C168) {
		...
		rt_spin_unlock_irqrestore(flags, &tp->mii_lock);
...
void __devinit tulip_find_mii (struct rtnet_device *rtdev, int board_idx)
{
	...
	for (phyn = 1; phyn <= 32 && phy_idx < sizeof (tp->phys); phyn++) {
		...
		/*RTnet*/rt_printk (KERN_INFO "tulip%d:  MII transceiver #%d "
	...
	if (tp->mtable && tp->mtable->has_mii && phy_idx == 0) {
		/*RTnet*/rt_printk (KERN_INFO "tulip%d: ***WARNING***: No MII transceiver found!\n",
			board_idx);
		tp->phys[0] = 1;
	}
}
