...
int __init fec_enet_init(void)
{
...
#if defined(CONFIG_SCC_ENET) && defined(CONFIG_RTAI_RTNET_MODULE)
	/*
	...
	*/
	...
	i = rt_request_global_irq_ext(FEC_INTERRUPT, 
				      (void (*)(void))fec_enet_interrupt, 
				      (unsigned long)rtdev);
	if (i) {
		MOD_DEC_USE_COUNT;
		return i;
	}
	rt_enable_irq(FEC_INTERRUPT);
