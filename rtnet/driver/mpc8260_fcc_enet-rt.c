...
#ifdef CONFIG_RTAI_RTNET_FCC1_ENET
...
#endif
#ifdef CONFIG_RTAI_RTNET_FCC2_ENET
...
#endif
#ifdef CONFIG_RTAI_RTNET_FCC3_ENET
...
#elif defined(CONFIG_PM826)
	*eap++ = dev->rtdev_addr[5] = bd->bi_enetaddr[5] + fip->fc_fccnum + 1;
        for (i=4; i>=0; i--) {
		*eap++ = rtdev->dev_addr[i] = bd->bi_enetaddr[i];
	}
#else
