...
#ifndef CONFIG_CPM_MULTILEVEL_IRQ
#error "Multi-level CPM interrupts not selected, please correct!"
#endif
...
#ifndef CONFIG_RTAI_RTNET_MODULE
static struct net_device_stats *scc_enet_get_stats(struct net_device *dev);
static void set_multicast_list(struct net_device *dev);
#endif

#ifdef CONFIG_RTAI_RTNET_MODULE
...
#endif /* CONFIG_RTAI_RTNET_MODULE */
...
#error Exactly ONE pair of PA_ENET_[RT]XD, PB_ENET_[RT]XD must be defined
	...
	*/
	...
	i = rt_request_global_irq_ext(rtdev->irq,
				      (void (*)(void))scc_enet_interrupt, 
				      (unsigned long)rtdev);
	if (i) {
...
#error Configuration Error: define exactly ONE of PB_ENET_TENA, PC_ENET_TENA
	...
	*/
	...
	printk("%s: CPM ENET Version 0.2 on SCC%d, %02x:%02x:%02x:%02x:%02x:%02x\n", 
	       rtdev->name, SCC_ENET+1, 
